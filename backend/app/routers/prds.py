...
async def stream_prd_generation(
    ...
    """Stream PRD generation results as Server-Sent Events (SSE).

    This endpoint streams the PRD generation section by section as they are
    generated by the LLM. The PRD is saved to the database after all sections
    are complete.

    Emits events:
    - {event: 'status', data: {status: 'generating', mode: '...'}} at start
    - {event: 'title', data: {title: '...'}} when title is extracted
    - {event: 'section', data: {id, title, content, order}} for each section
    - {event: 'complete', data: {prd_id, version, section_count}} when done
    ...
    async def event_generator() -> AsyncIterator[dict[str, Any]]:
        """Generate SSE events for PRD streaming."""
        try:
            ...
            # Create generator and stream sections
            ...
            async for event in generator.generate_stream(
                ...
                if event_type == "title":
                    yield {
                        "event": "title",
                        "data": json.dumps({"title": event["title"]}),
                    }
...
async def stream_prd_generation_v2(
    project_id: str,
    request: Request,
    mode: PRDMode = Query(default=PRDMode.DRAFT, description="Generation mode (draft or detailed)"),
    db: Session = Depends(get_db),
    ...
    This v2 endpoint implements section-by-section generation:
